export const DropDownData = [
  { id: 1, dropDownMenu: '스마트모빌리티' },
  { id: 2, dropDownMenu: '스포츠' },
  { id: 3, dropDownMenu: '라이프스타일' },
  { id: 4, dropDownMenu: '주니어' },
];

export const DropDownData2 = [
  { id: 17, dropDownMenu2: '전기자전거' },
  { id: 20, dropDownMenu2: '로드자전거' },
  { id: 'id', dropDownMenu2: '픽시' },
  { id: 'df', dropDownMenu2: '주니어' },
];

export const Brand = [
  { id: 5, brandList: '팬텀 ' },
  { id: 6, brandList: '아팔란치아' },
  { id: 7, brandList: '레스포' },
  { id: 8, brandList: '콜라보레이션' },
];

export const hashTag = [
  { id: 9, hashTagList: '#전기자전거' },
  { id: 10, hashTagList: '#로드자전거' },
  { id: 11, hashTagList: '#픽시' },
  { id: 12, hashTagList: '#어린이자전거' },
  { id: 13, hashTagList: '#출퇴근용' },
  { id: 14, hashTagList: '#통학용' },
  { id: 15, hashTagList: '#여성용' },
  { id: 16, hashTagList: '#여행용' },
];
